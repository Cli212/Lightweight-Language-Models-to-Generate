...
class QADataset(Dataset):
    def __init__(self, data: List[Tuple[List[Any], List[Any]]], pad, sos, eos, max_length, proj_feature_size=None, voc=None):
        self.sos_tag = sos
        self.eos_tag = eos
        self.pad = pad
    ...
    def __getitem__(self, item):
        question, answer = zeroPadding([self.sos_tag] + self.data[item][0][:self.max_length-2] + [self.eos_tag],
                                       self.max_length, self.pad), zeroPadding([self.sos_tag] + self.data[item][1][:self.max_length-2] + [self.eos_tag], self.max_length, self.pad)
...
def create_dataloaders(df, sos, eos, pad, max_length, batch_size, train_size=0.8, proj_feature_size=None, voc=None, seed=123):
